...
public class Events {
    ...
    public static boolean link(OdinPlayer player, String name) {
        ...
        if (!event.isCancelled() && !player.isAuthenticated() && !player.isLinked()) {
            player.login();
            if (!OdinManager.userLinkedNames.containsKey(player.getUsername())) {
                OdinManager.userLinkedNames.put(player.getUsername(), name);
            }
            if (OdinManager.cfgMgr.getBoolean("link.rename")) {
